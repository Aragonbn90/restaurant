...
export const Dish = (props: IDishProps) => {
  ...
  const { dishList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="dish-heading">
        Dishes
      ...
      <div className="table-responsive">
        {dishList && dishList.length > 0 ? (
            ...
            <tbody>
              {dishList.map((dish, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{dish.orderId ? <Link to={`order/${dish.orderId}`}>{dish.orderId}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${dish.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${dish.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && <div className="alert alert-warning">No Dishes found</div>
        )}
      </div>
